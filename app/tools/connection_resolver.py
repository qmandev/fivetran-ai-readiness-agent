...
Canonical home for the resolver. It lives under ``app/`` (not ``ingest/``)
because the Agent Runtime wheel only packages ``["app","frontend"]`` — the
``ingest`` namespace package is NOT deployed there. The v3 readiness tools call
this at runtime, so it must ship inside ``app``. The Cloud Run webhook receiver
has ``app/`` on its path too, so ``ingest.webhook_receiver.connection_resolver``
...
Cache: module-level dict; survives the instance lifetime. The set of
       connection_ids per instance is tiny so eviction is unnecessary.
